...
public class DTNHost implements Comparable<DTNHost> {
	...
	public boolean addRecord( String dataName,String rquestNodeName,double popularity,String timestamp ){
		Map<String,Object> record = new HashMap<String ,Object>();
		List<String> from = new ArrayList<String>();
		from.add(rquestNodeName);
		record.put("dataName",dataName);
		record.put("rquestNodeName",from);
		record.put("popularity",popularity);
		record.put("timestamp",timestamp);
		for(Map i :PIT){
			if( i.containsValue(dataName)){
				List<String>  rquestNodeNameValue =  (List<String>) i.get("rquestNodeName");
				if(!rquestNodeNameValue.contains(rquestNodeName)){
					rquestNodeNameValue.add(rquestNodeName);
	...
	public Message initData(Message m){
		Message newMessage = m.replicate();
		 this.router.getDeliveredMessages().put(m.getId(),newMessage);
		//cache listener
		for (MessageListener ml : this.router.getmListeners()) {
			ml.newMessage(m);
		}
		return newMessage;
