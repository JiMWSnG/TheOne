...
public class Osm2Wkt {
    ...
    private class OsmHandler extends DefaultHandler {
        ...
        public void startDocument() throws SAXException {
            ...
            System.out.println("开始解析osm文件/t/n");
        }
...
//            System.out.print("开始解析第"+num+"条数据");
           //判断root 是否是osm
            ...
            if (!isOsm &&!XML_TAG_OSM.equals(qName)) {
                System.out.println("invalid osm file, root element is "
                        + qName
                        + " but should be " + XML_TAG_OSM);
                return;
            }
            //node  read in all landmarks
            if (XML_TAG_NODE.equals(qName)) {
                if (attributes != null) {
                    String idAttr = null, idLat = null, idLon = null;
                    for (int i = 0; i < attributes.getLength(); i++) {
                        if (attributes.getQName(i).equals(XML_TAG_ID)) {
                            idAttr = attributes.getValue(i);
                        } else if (attributes.getQName(i).equals(XML_TAG_LAT)) {
                            idLat = attributes.getValue(i);
                        } else if (attributes.getQName(i).equals(XML_TAG_LON)) {
                            idLon = attributes.getValue(i);
                        }
                    ...
                    landObj.id = Long.valueOf(idAttr);
                    landObj.latitude = Double.valueOf(idLat);
                    landObj.longitude = Double.valueOf(idLon);

                    landmarks.put(landObj.id, landObj);
                }
            }

            // way  read in all streets
            else if (XML_TAG_WAY.equals(qName)) {
                isWay = true;
                String streetId = null;
                if (attributes != null&&attributes.getQName(0).equals(XML_TAG_ID)){
                    streetId = attributes.getValue(0);
                    lastStreetId = Long.valueOf(streetId);
                }else{
                    System.out.println("missing attribute in landmark "
                            + attributes.toString());
                }
            }
            else if (XML_TAG_ND.equals(qName)){
                String ref = null;
                if (attributes.getQName(0).equals(XML_TAG_REF)){
                    ref = attributes.getValue(0);
                }else{
                    System.out.println("missing attribute in street landmark "
                            + attributes.toString());
                }
                if (ref != null){
                    streetLandMarks.add(Long.valueOf(ref));
                }
        ...
        public void endElement(String uri, String localName, String qName) throws SAXException {
            if (XML_TAG_WAY.equals(qName)){
                isWay =false;
                streets.put(lastStreetId, streetLandMarks);
                streetLandMarks = new Vector<>();
            }
        }
    ...
    private boolean readOsmBySax(String filename) {
